...
def load_overall(path: str) -> pd.DataFrame:
    ...
    cecl_overall = os.path.join(
        path, "cecl_multi", "portfolio_aggregates_overall.parquet"
    )
    ...
    if os.path.exists(by_product_root):
        ...
        overall = (
            bp.groupby("asof_month", as_index=False)[
                ["portfolio_monthly_ecl", "portfolio_ead"]
            ]
            .sum()
...
def main() -> None:
    ...
    for r in runs:
        ...
        last_cov = (
            overall["portfolio_monthly_ecl"].iloc[-1]
            / overall["portfolio_ead"].iloc[-1]
        ) * 100.0
        records.append(
            {
