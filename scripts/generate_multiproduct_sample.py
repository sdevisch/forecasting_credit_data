...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate multi-product synthetic dataset sample"
    )
    ...
    parser.add_argument(
        "--partitioned", action="store_true", help="Write partitioned Parquet outputs"
    )
    parser.add_argument(
        "--products",
        type=str,
        default="card,auto,personal,mortgage,heloc",
        help="Comma-separated products to include",
    )
    ...
    with timed(logger, "generate_loans_panels"):
        if "card" in products:
            ...
            panels_map["card"] = _simulate_card_panel(
                loans_map["card"], macro, months=args.months, seed=args.seed
            )
        if "auto" in products:
            loans_map["auto"] = generate_auto_loans(borrowers, seed=args.seed)
            panels_map["auto"] = simulate_auto_panel(
                loans_map["auto"], macro, months=args.months, seed=args.seed
            )
        if "personal" in products:
            loans_map["personal"] = generate_personal_loans(borrowers, seed=args.seed)
            panels_map["personal"] = simulate_personal_panel(
                loans_map["personal"], macro, months=args.months, seed=args.seed
            )
        if "mortgage" in products:
            loans_map["mortgage"] = generate_mortgage_loans(borrowers, seed=args.seed)
            panels_map["mortgage"] = simulate_mortgage_panel(
                loans_map["mortgage"], macro, months=args.months, seed=args.seed
            )
        if "heloc" in products:
            loans_map["heloc"] = generate_heloc_loans(borrowers, seed=args.seed)
            panels_map["heloc"] = simulate_heloc_panel(
                loans_map["heloc"], macro, months=args.months, seed=args.seed
            )
    ...
    with timed(logger, "write_outputs"):
        ...
        if args.partitioned:
            loans_all = pd.concat(
                [df.assign(product=prod) for prod, df in loans_map.items()],
                ignore_index=True,
            )
            loans_all.to_parquet(
                os.path.join(out_path, "loans_partitioned.parquet"),
        ...
        if args.partitioned:
            panels_all = pd.concat(
                [df.assign(product=prod) for prod, df in panels_map.items()],
                ignore_index=True,
            )
            panels_all.to_parquet(
                os.path.join(out_path, "loan_monthly_partitioned.parquet"),
                partition_cols=["product", "asof_month"],
            )
        else:
