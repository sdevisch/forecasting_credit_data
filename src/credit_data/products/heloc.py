...
def generate_heloc_loans(
    ...
    rate = np.clip(
        0.055
        + (700 - borrowers["fico_baseline"]).clip(0) * 0.00015
        + rs.normal(0, 0.005, n),
        0.03,
        0.18,
    )
    ...
    df = pd.DataFrame(
        {
            ...
            "product": "heloc",
            "origination_dt": pd.to_datetime("2018-01-01")
            + pd.to_timedelta(rs.integers(0, 365 * 4, n), unit="D"),
...
def simulate_heloc_panel(
    ...
    util0 = np.clip(
        loans["orig_balance"].to_numpy() / loans["credit_limit"].to_numpy(), 0.05, 0.95
    )
    util_shocks = (
    ...
    for t_idx, asof in enumerate(panel_months):
        ...
        p_c_to_30 = np.clip(
            (0.008 + 0.00005 * risk + 0.008 * max(0.0, udev)) * seasoning, 0.001, 0.22
        )
        ...
        bal_t = np.where(
            state == 4, 0.0, np.clip(bal_t - payment - prepay_amt, 0.0, None)
        )

        lgd_t = np.where(
            state == 4,
            np.clip(0.4 + 0.2 * hpi_down + 0.03 * max(0.0, udev), 0.2, 0.9),
            0.5,
        )
