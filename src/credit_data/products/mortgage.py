...
def generate_mortgage_loans(
    ...
    rate = np.clip(
        0.035
        + (700 - borrowers["fico_baseline"]).clip(0) * 0.00005
        + rs.normal(0, 0.003, n),
        0.02,
        0.08,
    )
    ...
    df = pd.DataFrame(
        {
            ...
            "product": "mortgage",
            "origination_dt": pd.to_datetime("2017-01-01")
            + pd.to_timedelta(rs.integers(0, 365 * 3, n), unit="D"),
...
def simulate_mortgage_panel(
    ...
    for t_idx, asof in enumerate(panel_months):
        ...
        p_c_to_30 = (
            np.clip(0.003 + 0.00003 * risk + 0.006 * max(0.0, udev), 0.0005, 0.12)
            * seasoning
        )
        ...
        lgd_t = np.where(
            state == 4,
            np.clip(0.35 + 0.15 * hpi_down + 0.03 * max(0.0, udev), 0.15, 0.8),
            np.nan,
        )
