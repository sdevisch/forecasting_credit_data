...
def simulate_auto_panel(loans: pd.DataFrame, macro: pd.DataFrame, months: int, seed: int | None = 12345) -> pd.DataFrame:
    ...
    # Default hazard influenced by fico and macro; LGD lower when unemployment is low (better recoveries)
    ...
    for t_idx, asof in enumerate(panel_months):
        # Hazards this month
        pd_t = np.clip(base_pd * (1.0 + 0.1 * max(0.0, unemp_dev[t_idx])), 0.0002, 0.08)
        u = rs.random(n)
        ...
        # Recoveries: fraction of balance at default with lag via schedule approximation
        lgd = np.clip(0.5 + 0.02 * unemp_dev[t_idx], 0.3, 0.8)
