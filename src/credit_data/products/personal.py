...
def generate_personal_loans(
    ...
    rate = np.clip(
        0.12
        + (700 - borrowers["fico_baseline"]).clip(0) * 0.0005
        + rs.normal(0, 0.01, n),
        0.08,
        0.36,
    )
    ...
    df = pd.DataFrame(
        {
            ...
            "product": "personal",
            "origination_dt": pd.to_datetime("2020-01-01")
            + pd.to_timedelta(rs.integers(0, 24, n), unit="D"),
...
def simulate_personal_panel(
    ...
    for t_idx, asof in enumerate(panel_months):
        ...
        pd_t = np.clip(
            base_pd
            * (0.8 + 0.5 * seasoning)
            * (1.0 + 0.12 * max(0.0, unemp_dev[t_idx])),
            0.0004,
            0.15,
        )
        u = rs.random(n)
        ...
        # Prepayment more sensitive to falling rates
        smm = np.clip(0.003 + 0.015 * np.clip(-fed_dev[t_idx], 0.0, None), 0.0, 0.08)
        prepay_amt = smm * balance

        balance = np.where(charged_off, 0.0, balance - principal - prepay_amt)

        lgd = np.clip(0.8 + 0.04 * max(0.0, unemp_dev[t_idx]), 0.6, 0.98)
        recovery = np.where(
