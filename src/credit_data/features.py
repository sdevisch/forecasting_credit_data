...
def add_lags(
    ...
    for c in cols:
        for lag in lags:
            out[f"{c}_lag{lag}"] = out.groupby(by, sort=False)[c].shift(lag)
    return out
    ...
    for c in cols:
        ...
        for w in windows:
            out[f"{c}_roll{w}_mean"] = (
                g.rolling(w, min_periods=1).mean().reset_index(level=by, drop=True)
            )
            out[f"{c}_roll{w}_std"] = (
                g.rolling(w, min_periods=1).std().reset_index(level=by, drop=True)
            )
    return out


def add_interactions(
    df: pd.DataFrame, interactions: List[Tuple[str, str, str]]
) -> pd.DataFrame:
