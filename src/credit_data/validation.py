        ...
        issues.append(
            {
                "level": "ERROR",
                "scope": f"loans_{product}",
                "message": "Negative orig_balance found",
            }
        )
        ...
        issues.append(
            {
                "level": "ERROR",
                "scope": f"loans_{product}",
                "message": "Negative interest_rate found",
            }
        )
    return issues
        ...
        issues.append(
            {
                "level": "ERROR",
                "scope": f"panel_{product}",
                "message": "Negative balance_ead found",
            }
        )
    # Null ratios
        ...
        issues.append(
            {
                "level": "WARN",
                "scope": f"panel_{product}",
                "message": f"balance_ead null ratio {null_ratio:.2%} > 1%",
            }
        )
    return issues
...
def validate_dataset_dir(dataset_dir: str) -> pd.DataFrame:
    ...
    for p in sorted(
        [
            ...
            if f.startswith("loans_") and f.endswith(".parquet")
        ]
    ):
    ...
    for p in sorted(
        [
            f
            for f in os.listdir(dataset_dir)
            if f.startswith("loan_monthly_") and f.endswith(".parquet")
        ]
    ):
    ...
    report = (
        pd.DataFrame(issues, columns=["level", "scope", "message"])
        if issues
        else pd.DataFrame(columns=["level", "scope", "message"])
    )
    return report
